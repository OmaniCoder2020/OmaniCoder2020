...
{
    sort();
    //  binary_searchHelper(SortingList<Term> terms, string key, int left_index, int right_index);
    return binary_searchHelper(terms, prefix,0,terms.size());
...
void Autocomplete::search(string key, int& firstIndex, int& lastIndex)
{
    firstIndex = -1;
    lastIndex = -1;
    int indexPoint = binary_search(key);
    // loop though the first Index 
    for(int i = indexPoint; i >= 0; i--){
    if(terms[i].compareByPrefix(terms[i],terms[indexPoint],key.length())!=0){
        firstIndex = i+1;
        break;
    }
    }

    for(int i= indexPoint; i < terms.size();i++){
        if(terms[i].compareByPrefix(terms[i],terms[indexPoint],key.length())!=0){
        lastIndex = i-1;
        break;
    }
...
SortingList<Term> Autocomplete::allMatches(string prefix)
{
    ...
    for(int i = firstIndex; i <= lastIndex;i++){
        matchedTerms.insert(this->terms[i]);
    }
